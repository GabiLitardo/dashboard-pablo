...
@st.cache_data(ttl=10)  # TTL bajo para permitir refrescos rápidos tras enviar datos
    ...
    with st.form("formulario_gsheets", clear_on_submit=True):
        ...
        if boton_enviar:
            ...
            nueva_fila = pd.DataFrame([{
                "fecha [DD/MM/YYYY]": input_fecha.strftime("%d/%m/%Y"),
                "actividad": input_actividad,
                "tiempo [hh:mm:ss]": input_tiempo,
                "distancia [Km]": input_distancia,
                "calorías [Kcal]": input_calorias,
                "ritmo cardíaco [BPM]": input_bpm
            }])
